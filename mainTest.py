...
def main():
    ...
    agentDesigner = AgentAI(server_address="http://deepthought.cse.chalmers.se", 
                            model_name="llama3.2",
                            my_role="You are a C designer. You will be given a task and you will respond with design suggestions to solve or the task.")
    
    agentAnalyzer = AgentStaticAnalyzer(server_address="http://deeperthought.cse.chalmers.se", 
                                              model_name="llama3.2", 
                                              trials=3)
    ...
    for i in tqdm(range(MAX_ITERATIONS), desc="Processing iterations"):
        ...
        agentProgrammer.save_to_excel("programmer_conversation_test_comp2.xlsx")
        agentAnalyzer.save_to_excel("static_analysis_log.xlsx")
